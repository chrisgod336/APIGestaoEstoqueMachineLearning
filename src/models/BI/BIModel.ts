        ...
        try {
            ...
            for (const idProduto of produtos) {
                ...
                try {
                    ...
                    const model = trainModel(dadosTreino);
            ...
            console.log('🧹 Limpando previsões anteriores...', 
                mesesParaLimpar.map(m => `${m.mes}/${m.ano}`).join(', '));
            ...
            for (const element of resultadoAgrupado) {
                for (const produto of element.produtos) {
                    ...
                    const produtoInfo = await db.get(
                    ...
                    if (produtoInfo) {
                        ...
                        operacoesInsercao.push(
                            db.run(
                                `INSERT OR REPLACE INTO tb_previsao_venda(mes, ano, id_produto, nu_quantidade, vr_total)
                                 VALUES(?,?,?,?,?)`,
                                [mes, ano, id_produto, nu_quantidade, vr_total]
                            ),
                            db.run(
                                `INSERT OR REPLACE INTO tb_previsao_compra(mes, ano, id_produto, nu_quantidade, vr_total)
                                 VALUES(?,?,?,?,?)`,
                                [mes, ano, id_produto, compras_necessarias, vr_total_compra]
                            ),
                            db.run(
                                `INSERT OR REPLACE INTO tb_previsao_estoque(mes, ano, id_produto, nu_quantidade, vr_total)
                                 VALUES(?,?,?,?,?)`,
                                [mes, ano, id_produto, estoque_necessario, vr_total_estoque]
                            )
                        );
                    }
                }
            ...
            await Promise.all(operacoesInsercao);
            await db.run('COMMIT');
